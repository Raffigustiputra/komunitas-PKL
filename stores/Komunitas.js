...
export const useKomunitas = defineStore('komunitas', () => {
    ...
    const fetchKomunitasDetail = async (komunitasId) => {
        try {
            const response = await fetch(`http://192.168.19.251:8000/api/communities/detail/${komunitasId}/`, {
                headers: {
            ...
            if (!response.ok) {
                throw new Error(`Gagal mengambil detail komunitas. Status: ${response.status}`);
            }
            ...
            return {
                ...
                image: responseData.image ? `http://192.168.19.251:8000${responseData.image}` : null,
                visibility: responseData.visibility,
                category: responseData.category ? responseData.category.name : "Tidak ada kategori",
            };
        } catch (err) {
    ...
    async function CreateKomunitas(name, description, image, visibility, category) {
        try {
            ...
            const response = await fetch('http://192.168.19.251:8000/api/communities/create/', {
                method: 'POST',
                body: formData,
                headers: {
    ...
    // async function JoinKomunitas(request, response)

    // Fungsi untuk mengambil kategori komunitas
    async function CategoryKomunitas() {
        try {
            const response = await fetch('http://192.168.19.251:8000/api/categories/');
            if (!response.ok) {
                throw new Error(`Gagal mengambil kategori komunitas. Status: ${response.status}`);
            }
    ...
    async function removeKomunitas(komunitasId) {
        try {
            const response = await fetch(`http://192.168.19.251:8000/api/communities/${komunitasId}/`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Token ${useAuth().userToken.value}`,
                },
            ...
            if (!response.ok) {
                throw new Error(`Gagal menghapus komunitas. Status: ${response.status}`);
            }
    ...
    async function removeImage(komunitasId) {
        try {
            const response = await fetch(`http://192.168.19.251:8000/api/communities/delete/${komunitasId}/`, {
                method: 'DELETE',
            });
            if (!response.ok) {
            ...
            console.error(`Error deleting image for community ID ${komunitasId}:`, err);
            throw err;
        }
    }
