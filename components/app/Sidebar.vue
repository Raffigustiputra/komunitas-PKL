<template>
  ...
  <nav class="hidden sm:flex w-64 h-screen px-4 py-6 flex-col fixed">
    ...
    <div class="mt-8 space-y-1">
      <BaseButtonSidebarButton :onClick="() => navigateTo('/homepage')" buttonName="Homepage" :icon="Homepage" path="/homepage" class="dark:text-white" />
      <BaseButtonSidebarButton :onClick="() => navigateTo('/community-list')" buttonName="Komunitas" :icon="Community" path="/community-list" class="dark:text-white"/>
      <BaseButtonSidebarButton :onClick="() => navigateTo('/search')" path="/search" buttonName="Pencarian" :icon="Search" class="dark:text-white"/>
      <BaseButtonSidebarButton :onClick="() => navigateTo('/bookmark')" path="/bookmark" buttonName="Bookmark" :icon="Bookmark"class="dark:text-white" />
      <BaseButtonSidebarButton buttonName="Notifikasi" :icon="Notification" class="dark:text-white"/>
      <BaseButtonSidebarButton :onClick="() => navigateTo('/pengaturan')" path="/pengaturan" buttonName="Pengaturan" :icon="Setting" class="dark:text-white"/>
      <BaseButtonSidebarButton :onClick="() => useAuth().logout()" buttonName="Logout" :icon="Logout" class="dark:text-red-400"/>
    </div>

    <!-- Tombol Posting -->
    <div class="flex flex-col mt-12 max-w-[30vh] sm:w-40 lg:w-60">
  ...
  <!-- Navigasi Mobile -->
  <nav class="fixed bottom-0 left-0 right-0 sm:hidden bg-white dark:bg-gray-900 p-3 border-t flex justify-around items-center z-36">
