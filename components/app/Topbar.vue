<template>
  <div v-if="useAuth().userToken.value" class="bg-white/80 backdrop-blur-sm dark:bg-black rounded-b-3xl sticky top-0 z-10">
    ...
    <Transition name="slide">
      <div v-if="showSlideMenu" class="fixed inset-0 bg-black/50 z-50 md:hidden" @click="toggleSlideMenu">
        <div class="absolute left-0 top-0 h-full w-72 bg-white dark:bg-black shadow-lg p-5 flex flex-col" @click.stop>
          <!-- Profile Section -->
          ...
          <button @click="navigateTo('/profile')" class="menu-item">👤 Profile</button>
          <button @click="navigateTo('/notifikasi')" class="menu-item">🔔 Notifikasi</button>
          <button @click="navigateTo('/pengaturan')" class="menu-item">⚙ Pengaturan</button>
